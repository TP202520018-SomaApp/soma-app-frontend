...
export default async function getUniversities() {
    try {
        const response = await requester.get('/universities');
            ...
            error.response?.data?.error ? error.response.data.error
                : error.response?.status === HttpStatusCode.Unauthorized ?
                    ...
                    : error.response?.status === HttpStatusCode.NotFound ?
                        "El sistema de universidades se encuentra temporalmente en mantenimiento"
                        : error.response?.status === HttpStatusCode.InternalServerError ?
                            "Error interno del servidor al obtener las universidades"
